...
namespace Event
{
    ...
    public partial class PageAccount : INotifyReceiver
    {
        ...
        private async void ButtonExit_OnClicked(object sender, EventArgs e)
        {
            ...
            if (!await DisplayAlert(Title, "Вы действительно собираетесь выйти из программы?", "OK", "ОТМЕНА"))
                return;
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    Save.ConnectionLogin = string.Empty;
                    Save.ConnectionPassword = string.Empty;
                    Config.MobEvent = null;
                    Config.User = null;
                });
