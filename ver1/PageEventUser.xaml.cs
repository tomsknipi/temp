...
namespace Event
{
    ...
    public partial class PageEventUser
    {
        ...
        private async Task<bool> RefreshData(bool nextPage)
        {
            ...
            if (Config.User != null)
            {
                var result = await Service.GetUserList(Config.User.Id, nextPage, UserLastName);
        ...
        private void UserListView_OnItemTapped(object sender, ItemTappedEventArgs e)
        {
            if (!(e.Item is MobUser item))
                return;
            Navigation.PushAsync(new PageEventUserCard(item));
        }
